...
Page({
  data: {
    loading: true,
    colorPalette: [] as Array<{ key: string; color: string }>,
    selectedColor: '' as string,
    selectedColorKey: '' as string,
    pixelData: null as MappedPixel[][] | null,
    gridDimensions: null as { N: number; M: number } | null
  },
  ...
  processImage() {
    ...
    img.onload = () => {
      ...
      this.setData({
        loading: false,
        colorPalette: colorPalette.map(color => ({ key: color.key, color: color.hex })),
        pixelData: pixelData,
        gridDimensions: { N, M }
      });
  ...
  // 生成颜色调色板
  generateColorPalette(): PaletteColor[] {
  ...
  // 生成模拟像素数据
  generateMockPixelData(N: number, M: number, colorPalette: PaletteColor[]): MappedPixel[][] {
    const pixelData: MappedPixel[][] = [];
  ...
  // 绘制像素化图像
  drawPixelatedImage(canvas: any, pixelData: MappedPixel[][], N: number, M: number, width: number, height: number) {
